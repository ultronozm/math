...
\end{equation*}
(We follow the treatment of \cite[\S2.6]{2015arXiv150601452B} (TODO: fix ref).)  A polynomial function on $(\mathbb{R} / 2 \pi i \log q)^n$ is a function of the form $s \mapsto p(q^{- s}, q^{s})$ for some $p \in \mathbb{C}[x, y] /(x y - 1)$.  Locally, $T_p(G(F))$ is a quotient of $(\mathbb{R} / 2 \pi i \log q)^n$ by a finite group.  A polynomial function on $T_p(G(F))$ is a function that pulls back to a polynomial function on $(\mathbb{R} / 2 \pi i \log q)^n$.  (You can ask me later to give an example.)  Define the basic function to be
\begin{equation*}
  b_X = \int_{T_p(G(F))}
