...
async function fetchCommentsForDiscussion(discussionNumber) {
  const response = await axios.get(`https://api.github.com/repos/${owner}/${repo}/discussions/${discussionNumber}/comments`, {
    headers: {
      Authorization: `token ${process.env.GITHUB_TOKEN}`,
      Accept: 'application/vnd.github.echo-preview+json',
    },
...
async function fetchRecentComments() {
  try {
    const discussions = await fetchRecentDiscussions();
    
    for (const discussion of discussions) {
      discussion.comments = await fetchCommentsForDiscussion(discussion.number);
    }

    return discussions.map(discussion => ({
      title: discussion.title,
      url: discussion.html_url,
      recentComments: discussion.comments.slice(-10).map(comment => ({
        id: comment.id,
        url: `${discussion.html_url}#discussioncomment-${comment.id}`,
        author: comment.user.login,
        createdAt: comment.created_at,
      })),
    }));
  } catch (err) {
    console.error('Error while fetching discussions or comments:', err);
    return [];
  }
}
